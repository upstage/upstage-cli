...
utils.readConfig = function readConfig (repo, configProp, cb) {
  ...
  var filenames = [
    ...
    'json:' + configProp + '.rc',
    'yaml:' + configProp + '.rc',
    'json:' + configProp + '.json',
    'yaml:' + configProp + '.yml',
    'yaml:' + configProp + '.yaml',
    'yaml:.' + configProp + '.yml',
    'yaml:.' + configProp + '.yaml',
    'json:package.json'
  ];
  ...
  var config = null;
  var len = filenames.length, i = 0;
  function found () {
    return config != null || (i >= len);
  }

  async.until(found, function (callback) {
    try {
      var filename = filenames[i++].split(':');
      config = configFile.load(filename[1], {cwd: cwd, parse: filename[0]})
      if (config && filename[1] === 'package.json') {
